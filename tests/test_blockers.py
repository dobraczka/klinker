...
from klinker.blockers.concat_utils import concat_values
...
def example_with_expected(
    request,
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, Type[Blocker]]:
    example, (expected, cls, index_prefix), stringify, use_dask = request.param
    ta, tb, _, _ = request.getfixturevalue(example)
    expected = request.getfixturevalue(expected)
    if use_dask:
        ta = dd.from_pandas(ta, npartitions=2)
        tb = dd.from_pandas(tb, npartitions=2)
...
def test_assign_schema_aware(example_with_expected, tmpdir):
    ...
    block = cls(blocking_key="BirthCountry").assign(
        ta,
        tb,
        left_id_col="head",
        right_id_col="head",
...
def test_assign_embedding_blocker(
    ...
    if use_dask and any(
        ...
        if eb != "hdbscan":
            ...
            assert all(
                block.blocks.compute().apply(len, axis=1) == eb_kwargs["n_neighbors"]
            )
...
def test_assign_relation_frame_encoder(
    ...
    a_ids = _get_ids(attr=ta.set_index("head"), rel=rel_ta)
    assert len(block) == len(a_ids)
    assert all(block.blocks.compute().apply(len, axis=1) == eb_kwargs["n_neighbors"])
